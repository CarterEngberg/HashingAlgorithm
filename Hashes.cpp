...
void StudentHash(const void * key, int len, uint32_t seed, void * out) {
	...
	//	if (LOG) { printf("digest: %08x - idx %u - %08x (printing is slow!)\n", digest, idx, u32data[idx]); }
		...
		// Variable that is set to the amount of rotation we want to do
		uint32_t rotation_amount = u32data[idx] % 32;

		// Rotate the digest left by the rotation amount
		digest = (digest << rotation_amount) ^ (digest >> (32 - rotation_amount));
